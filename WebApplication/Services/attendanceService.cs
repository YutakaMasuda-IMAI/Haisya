...
namespace WebApplication.Services
{
    public class AttendanceService : IAttendanceService
    {
        ...
        public async Task<bool> PostRegisterHoliday(HaisyaDataModel.AttendanceModalViewModel model)
        {
            ...
            DateOnly databaseCompliantDateTime = new DateOnly(model.Date.Year, model.Date.Month, model.Date.Day);
        ...
        /// </remarks>
        public async Task<HaisyaDataModel.AttendanceModalViewModel> GetAttendanceDataAsync(int driverId, DateOnly date, int employeeNumber)
        {
            // 引数で受け取った日付を、データベースで扱うのに適した形式（00:00:00の時刻）に変換
            DateOnly databaseCompliantDateTime = new DateOnly(date.Year, date.Month, date.Day);
    ...
    public interface IAttendanceService
    {
        ...
        /// <param name="employeeNumber">従業員番号。</param>
        /// <returns>Dto.AttendanceModalViewModelオブジェクトを含むTask。</returns>
        Task<HaisyaDataModel.AttendanceModalViewModel> GetAttendanceDataAsync(int driverId, DateOnly date, int employeeNumber);
    }
}