...
//>>docs: http://api.jqueryui.com/ympicker/
//>>demos: http://jqueryui.com/ympicker/
//>>css.structure: ../../themes/base/core.css
//>>css.structure: ../../themes/base/ympicker.css
//>>css.theme: ../../themes/base/theme.css


$.extend($.ui, { ympicker: { version: "1.13.2" } });
...
function Ympicker() {
	...
	this._mainDivId = "ui-ympicker-div"; // The ID of the main ympicker division
	...
	this._defaults = { // Global defaults for all the date picker instances
		...
		onUpdateYmpicker: null, // Define a callback function when the ympicker is updated
...
$.extend(Ympicker.prototype, {
	...
	_attachments: function (input, inst) {
		...
		input.off("focus", this._showYmpicker);
		...
		if (showOn === "focus" || showOn === "both") { // pop-up date picker when in the marked field
			input.on("focus", this._showYmpicker);
		}
	...
	_destroyYmpicker: function (target) {
		...
		if (ympicker_instActive === inst) {
			ympicker_instActive = null;
			this._curInst = null;
		}
	},
	...
	_updateYmpicker: function (inst) {
		...
		var origyearshtml,
			...
			onUpdateYmpicker = $.ympicker._get(inst, "onUpdateYmpicker");

		if (activeCell.length > 0) {
			ympicker_handleMouseover.apply(activeCell.get(0));
		}
		...
		if (inst === $.ympicker._curInst && $.ympicker._ympickerShowing && $.ympicker._shouldFocusInput(inst)) {
			inst.input.trigger("focus");
		}
		...
		if (onUpdateYmpicker) {
			onUpdateYmpicker.apply((inst.input ? inst.input[0] : null), [inst]);
		}
	},
		...
		inst.dpDiv.find("[data-handler]").map(function () {
			var handler = {
				...
				selectDay: function () {
					...
					$.ympicker._selectDay(id, +this.getAttribute("data-month"), +this.getAttribute("data-year"), 1, this);
					return false;
				},
		...
		for (row = 0; row < numMonths[0]; row++) {
			...
			*/
				...
				for (var dRow = 0; dRow < 3; dRow++) { // create date picker rows
					...
					for (dow = 0; dow < 4; dow++) { // create date picker days
						...
						tbody += "<td class='" +
							...
							(unselectable ? "" : " data-handler='selectDay' data-event='click' data-d='" + printDate.getDate() + "' data-month='" + printDate.getMonth() + "' data-year='" + printDate.getFullYear() + "'") + ">" + // actions
							(otherMonth && !showOtherMonths ? "&#xa0;" : // display for other months
								(unselectable ? "<span class='ui-state-default'>" + printDate.getDate() + "</span>" : "<a class='ui-state-default" +
									...
									//"'>" + monthNamesShort[printDate.getMonth()] + "</a>")) + "</td>"; // display selectable date
 ...
 * Global ympicker_instActive, set by _updateYmpicker allows the handlers to find their way back to the active picker.
 */
function ympicker_bindHover(dpDiv) {
	...
	return dpDiv.on("mouseout", selector, function () {
		...
		.on("mouseover", selector, ympicker_handleMouseover);
}

function ympicker_handleMouseover() {
	if (!$.ympicker._isDisabledYmpicker(ympicker_instActive.inline ? ympicker_instActive.dpDiv.parent()[0] : ympicker_instActive.input[0])) {
...
$.fn.ympicker = function (options) {
	...
	if (!$.ympicker.initialized) {
		$(document).on("mousedown", $.ympicker._checkExternalClick);
		$.ympicker.initialized = true;
	}

	/* Append ympicker main container to body if not exist. */
...
$.ympicker.version = "1.13.2";

var widgetsYmpicker = $.ympicker;
