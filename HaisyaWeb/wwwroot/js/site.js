...
async function alertCustomInputError(text) {
    await swal.fire({ title: "入力エラー", html: text, icon: "error" });
};

async function alertInputError(text) {
    await swal.fire({ title: "入力エラー", text: text, icon: "error" });
};

async function alertError(text, title) {
    await swal.fire({ title: title, text: text, icon: "error" });
};

async function alertError(text) {
    await swal.fire({ text: text, icon: "error" });
};

async function alertInfo(text, title) {
    await swal.fire({ title: title, text: text, icon: "info" });
};

async function alertInfo(text) {
    await swal.fire({ text: text, icon: "info" });
};

async function alertWarning(text, title) {
    await swal.fire({ title: title, text: text, icon: "warning" });
};

async function alertWarning(text) {
    await swal.fire({ text: text, icon: "warning" });
};

async function alertSuccess(text, title) {
    await swal.fire({ title: title, text: text, icon: "success" });
};
...
async function alertQuestion(text, title) {
    await swal.fire({ title: title, text: text, icon: "question" });
};

async function alertQuestion(text) {
    await swal.fire({ text: text, icon: "question" });
};
...
async function alertYesNoMsg(title, text, icon, confirmButtonText, cancelButtonText, html) {
    ...
    await Swal.fire({
        title: title, text: text, icon: icon, html: html, showCancelButton: true, confirmButtonText: confirmButtonText, cancelButtonText: cancelButtonText
    }).then((result) => {
...
async function alertThreeButtonsMsg(title, text, icon, confirmButtonText, cancelButtonText, denyButtonText) {
    ...
    await Swal.fire({
        title: title, text: text, icon: icon, showCancelButton: true, showDenyButton: true, confirmButtonText: confirmButtonText, cancelButtonText: cancelButtonText, denyButtonText: denyButtonText,
    }).then((result) => {
        if (result.isConfirmed) {
            resultVal = 1;
...
function DailogOpen(id, title, width, height, closeFunction, closeFunctionForString) {
    ...
    thedialogC = $("#" + id).dialog({
        ...
        open: function () { //Xボタン非表示
            ...
            $(".ui-dialog-titlebar-close", $(this).closest(".ui-dialog")).attr('id', 'btnDialogClose');
            ...
            $('#btnDialogClose').addClass('btnClose');
            $('#btnDialogClose').css('float', 'right');
            $('#btnDialogClose').css('width', '85px');

            $(".ui-dialog-title", $(this).closest(".ui-dialog")).addClass('fs-4');
            $(".ui-dialog-title", $(this).closest(".ui-dialog")).css('width', '50%');

            $("#btnDialogClose", $(this).closest(".ui-dialog")).on('click', function () {
                $("#" + id).html('');
